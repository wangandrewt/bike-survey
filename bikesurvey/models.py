...
class RandomPrimaryIdModel(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        """
        Call the super class' __init__ method and initialize a few vars.
        """
    ...
    def _make_random_key(self, key_len):
        ...
        """
        return ''.join([random.choice(self._IDCHARS)
                        for dummy in xrange(0, key_len - 1)])

    def save(self, *args, **kwargs):
        """
        Modified save() function, which selects a special unique ID if
        necessary.
        Calls the save() method of the first model.Models base class it can
        find in the base-class list.
        """
        ...
        while try_key_len <= self.CRYPT_KEY_LEN_MAX:
            ...
            # Apparently, this key is already in use
                ...
                # Only way to differentiate between different IntegrityErrors
                # is to look into the message string. Too bad. But I need to
                # make sure I only catch the ones for the 'id' column.
                #
                # Sadly, error messages from different databases look different
                # and Django does not normalize them. So I need to run more
                # than one test. One of these days, I could probably just
                # examine the database settings, figure out which DB we use
                # and then do just a single correct test.
                #
                # Just to complicates things a bit, the actual error message is
                # not always in e.message, but may be in the args of the
                # exception. The args list can vary in length, but so far it
                # seems that the message is always the last one in
                # the args list. So, that's where I get the message string
                # from. Then I do my DB specific tests on the message string.
                #
                msg = e.args[-1]
                if msg.endswith(
                        "for key 'PRIMARY'") or \
                        msg == "column id is not unique" \
                        or "Key (id)=" in msg:
                    ...
                    if try_since_last_key_len_increase == try_key_len:
                        ...
                        # but then try more and more for larger key sizes.
                        try_key_len += 1
                        try_since_last_key_len_increase = 0
                else:
            ...
            raise IntegrityError(
                "Could not produce unique ID for model of type %s" %
                type(self))
...
class SurveyInstance(RandomPrimaryIdModel):
    LOCATION_CHOICES = (
        ...
        ('Mowatt Lane Garage Exit @ Knox Rd.',
         'Mowatt Lane Garage Exit @ Knox Rd.'),
        ('Mall @ Woods Hall', 'Mall @ Woods Hall'),
        ('Regents Dr. @ Main Administration',
         'Regents Dr. @ Main Administration'),
...
class Biker(RandomPrimaryIdModel):
    ...
    bikerGender = models.CharField(
        max_length=1, choices=GENDER_CHOICES, default=None)
    bikerHelmet = models.CharField(
        max_length=1, choices=HELMET_CHOICES, default=None)
    bikerLocation = models.CharField(
        max_length=30, choices=LOCATION_CHOICES, default=None)
    time = models.TimeField(auto_now_add=True)
    date = models.DateField(auto_now_add=True)

    def __unicode__(self):
        return "Gender: " + self.bikerGender + ", Helmet: " + \
            self.bikerHelmet + ", Location: " + self.bikerLocation + " at " + \
            self.date.strftime(
                '%Y-%m-%d') + " at " + self.time.strftime('%H:%M')
